...
f7-page.rf_cr_2(:page-content="false")
  ...
  filters(v-if=" sd " @sd=" sd = $event ")
    ...
    LSP(v-else=" sd === 3 " @s=" s = $event "  @done=" __setCall({fn: '__closeSD'}) ")
  ...
  .page-content.ptr-content.infinite-scroll-content(ptr-mousewheel="true" @ptr:refresh=" refresh " @infinite="loadMore" :infinite-distance="50")
    ...
    f7-card(v-for=" (v, i) in data " :key="i")
      f7-card-content
        .flex.ft_13(@click=" __go('/rfs/cr/lrd/', {props: v}) ")
          .a
            .hlh_30.ft_15 {{ v.bankName }} 
            .lh_20.text-color-gray {{ v.payerTime }}

          .b.t_r
            .v_m.inlb
              .lh_30 {{ v.payerRealAmount._f3() }}
              .lh_20(:class=" config.stateClass[v.isDone] ") {{ v.isDone }}
            .inlb.w_5
...
import LSP from '@/components/filters/LSP'
...
export default {
  ...
  methods: {
    ...
    list (option = {pageNum: 1, page: 1, pageSize: this.pageSize, size: this.pageSize}, cb = this.defaultListCb) {
      this.$.get(api.qryRecharge, Object.assign({
        status: this.s.id,
        startDate: this.stet[0]._toAllString(),
        endDate: this.stet[1]._toAllString(),
      }, option)).then(({data: {payRecordData, totalSize}}) => {
        this.data = [...(option.page > this.fpage ? this.data : []), ...payRecordData]
